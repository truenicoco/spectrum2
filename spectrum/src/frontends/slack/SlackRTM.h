...
class SlackRTM {
	public:
		...
		boost::SWIFTEN_SIGNAL_NAMESPACE::signal<void ()> onRTMStarted;

		SlackAPI *getAPI() {
			return m_api;
		}

		boost::SWIFTEN_SIGNAL_NAMESPACE::signal<void (const std::string &channel, const std::string &user, const std::string &text, const std::string &ts)> onMessageReceived;
